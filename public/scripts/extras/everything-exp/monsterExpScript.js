        ...
        .then(data => {
            ...
            if(data.err) {
                ...
                Object.keys(data.monsterListByMap).forEach((mapId, mapIndex) => {
                    ...
                    let monsterExpTable = document.getElementById(`${mapId}-monster-exp-table`);

                    data.monsterListByMap[mapId].monsterList.forEach((monster, monsterIndex) => {
                        monsterExpTable.insertAdjacentHTML('beforeend', populateMonsterTables(charLevel, mapIndex, monster, monsterIndex, expMultiplier));
                    })
                })
            }
        })
...
// Check if presently in dark mode and add relevant CSS class if required
function createMonsterTables(mapId, mapData) {
    let hasDarkModeClass = document.body.classList.contains("dark-mode");
    let darkModeCss = hasDarkModeClass ? "table-dark" : "";

    return `<table class="font-small size-720 w-100 table table-sm table-bordered table-hover my-3 ${darkModeCss}">
        <thead>
            <tr>
                <th colspan="3">
                    <div class="font-subsubheader d-flex justify-content-center align-items-center">
                        <img src="${mapData.mapIconImgUrl}" class="mr-2"> ${mapData.mapIconName}
                    </div>
                </th>
            </tr>
            <tr>
                <th scope="col" class="align-middle">Monster</th>
                <th scope="col" class="table-cols-50 align-middle text-center">No. Kills</th>
                <th scope="col" class="col-5 align-middle text-center">EXP Breakdown</th>
            </tr>
        </thead>
        <tbody id='${mapId}-monster-exp-table'>
            
        </tbody>
    </table>`;
}

// Generates single table row (representing one monster) of data
// If monster is boss, highlight in blue
...
function populateMonsterTables(charLevel, mapIndex, monster, monsterIndex, expMultiplier) {
    ...
    return `<tr>
        <td class="${monster.isBoss ? "text-custom font-weight-bold" : "" } align-middle position-relative">
            ...
            <div class="monster-list-table-details w-100 h-100 d-flex flex-column justify-content-center px-2 position-absolute">
                <p class="font-weight-bold mb-0">[Lv. ${monster.monsterLevel}] ${monster.monsterName}</p>
                <p class="mb-0 ml-2">- Monster HP: ${monster.monsterHP.toLocaleString('en-SG')}</p>
        ...
        <td class="${monster.isBoss ? "text-custom font-weight-bold" : "" } align-middle text-center">
            <div class="d-flex flex-column">
                <div class="d-flex">
                    ...
                    <div class="col-6 px-2 text-left" id="map-${mapIndex}-monster-${monsterIndex}-base-exp" data-base-exp=${monster.monsterEXP}>${monster.monsterEXP.toLocaleString("en-SG")} EXP</div>
                </div>
                <div class="d-flex">
                    <div class="col-6 px-2 text-right">After Base Bonus</div>
                    <div class="col-6 px-2 text-left">${Math.round(monster.monsterEXP * levelDiffModifier).toLocaleString("en-SG")} EXP</div>
                </div>
                <div class="d-flex">
                    <div class="col-6 px-2 text-right">After EXP Buffs</div>
                    <div class="col-6 px-2 text-left">${Math.round(Math.round(monster.monsterEXP * levelDiffModifier) * expMultiplier).toLocaleString("en-SG")} EXP</div>
                </div>
                <div>-----</div>
